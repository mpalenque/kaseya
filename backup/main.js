// Main.js - Compatibility redirect to restructured version// Main.js - Compatibility redirect to restructured version// Main.js - Compatibility redirect to restructured version

// This file now redirects to the new modular structure

// This file now redirects to the new modular structure// This file now redirects to the new modular structure

console.log('Loading restructured Kaseya application...');



// Load the new restructured main file immediately

(function() {console.log('Loading restructured Kaseya application...');console.log('Loading restructured Kaseya application...');

  const script = document.createElement('script');

  script.src = 'main-restructured.js';

  script.onload = function() {

    console.log('Restructured application loaded successfully');// Load the new restructured main file// Load the new restructured main file

  };

  script.onerror = function() {(function() {(function() {

    console.error('Failed to load restructured application');

    const errorDiv = document.createElement('div');  const script = document.createElement('script');  const script = document.createElement('script');

    errorDiv.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:rgba(255,0,0,0.9);color:white;padding:20px;border-radius:10px;text-align:center;z-index:1000;max-width:90vw;font-family:system-ui,sans-serif;';

    errorDiv.innerHTML = 'Error loading application.<br>Please refresh the page.';  script.src = 'main-restructured.js';  script.src = 'main-restructured.js';

    document.body.appendChild(errorDiv);

  };  script.async = false; // Load synchronously to maintain execution order  script.async = false; // Load synchronously to maintain execution order

  document.head.appendChild(script);

})();  script.onload = function() {  script.onload = function() {

    console.log('Restructured application loaded successfully');    console.log('Restructured application loaded successfully');

  };  };

  script.onerror = function() {  script.onerror = function() {

    console.error('Failed to load restructured application');    console.error('Failed to load restructured application');

    const errorMsg = document.createElement('div');    const errorMsg = document.createElement('div');

    errorMsg.style.cssText = `    errorMsg.style.cssText = `

      position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%);      position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%);

      background: rgba(255,0,0,0.9); color: white; padding: 20px; border-radius: 10px;      background: rgba(255,0,0,0.9); color: white; padding: 20px; border-radius: 10px;

      text-align: center; z-index: 1000; max-width: 90vw;      text-align: center; z-index: 1000; max-width: 90vw;

      font-family: system-ui, sans-serif;      font-family: system-ui, sans-serif;

    `;    `;

    errorMsg.innerHTML = 'Error loading application.<br>Please refresh the page.';    errorMsg.innerHTML = 'Error loading application.<br>Please refresh the page.';
